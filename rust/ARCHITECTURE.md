...
Nineteen `extern "C"` exports. Two are trivial (`spt_native_abi_version`, `spt_buf_free`); eleven take a UTF-8
...
table as UTF-8 JSON and buffers a parse error, or panic text since ABI 18. The last four are the log pipeline
(`spt_logger_init`, `spt_logger_reinit`, `spt_log_emit`, `spt_logger_close`): `spt_logger_init` takes the raw
`sptLogger.json` bytes and hands back a buffer only on failure, `spt_log_emit` passes one line's fields
directly rather than a JSON document, and `spt_logger_close` takes nothing; `spt_logger_reinit` swaps the
running pipeline's configuration in place — parse failure leaves it untouched, and a same-path target reopens
in append mode instead of cascading the archives again — see *The log pipeline* below.
