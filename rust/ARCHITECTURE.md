...
| `bot_generator.rs` | `Generators/Bot/BotGenerator.cs` | The batch path's prelude draws, since ABI 38: health, skills, exp-reward-for-kill, voice, appearance (incl. the fixed-hands lookup), the PMC game-version/member-category block, and `GenerateBotFinish`'s dogtag. Run per bot between the level draw and the inventory. Naming and the sim-pscav cluster stay C#-side on every arm; the per-bot and player-scav exports keep their C# prelude |
